...
describe("protected dashboard profile images", () => {
  it("accepts only a small, valid image from an active administrator session and stores it through managed storage", () => {
    ...
    expect(dashboardHtml).toContain('id="dashboardProfileAvatarImage"');
	    ...
	    expect(adminSource).toContain('const [topLeftProfileImage, setTopLeftProfileImage] = useState(dashboardLogoUrl)');
	    expect(adminSource).toContain('type !== "91dab-dashboard-profile-brand-image"');
	    expect(adminSource).toContain('src={topLeftProfileImage} alt="Dashboard profile" className="h-9 w-9');
	    ...
	    expect(dashboardHtml).toContain('id="dashboardBrandImage"');
	    expect(dashboardHtml).toContain("type:'91dab-dashboard-profile-brand-image'");
